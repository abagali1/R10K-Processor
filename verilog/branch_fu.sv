...
module branch_fu (
    ...
    always_ff @(posedge clock) begin
        if (reset) begin
            fu_pack     <= '{result: '0, decoded_vals: '0, pred_correct: '1, rs2_value: 0, ld_state: LOAD_STATE'(0), target_addr: '0};
            data_ready  <= '0;
            br_task     <= NOTHING;
            ...
            fu_pack     <= '{result: is_pack.decoded_vals.decoded_vals.PC + 4, decoded_vals: out, pred_correct: correct, rs2_value: 0, ld_state: LOAD_STATE'(0), target_addr: target};
            data_ready  <= 1;
